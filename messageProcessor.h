...
//namespace training{
    ...
    public:
        ...
        static training::St_SERVER_OR_SIMULATOR_SIGN_ACK set_ACK(uint8_t code, uint16_t serviceFlag){
            training::St_SERVER_OR_SIMULATOR_SIGN_ACK ack;
        ...
        static void print_ACK(training::St_Payload_ACK payload_ack){
            printf("[ACK]\n");
            printf("Code: %d\n", (int)payload_ack.Code);
            printf("ServiceFlag: %d\n", (int)payload_ack.ServiceFlag);
        }

        static training::St_Payload_ACK parse_Payload_ACK(uint8_t* payload_buf){
            training::St_Payload_ACK payload;
        ...
        static void print_SIMULATOR_TX_payload(training::St_Payload_SIMULATOR_TX_J2735_DATA payload){
        ...
        static training::St_Payload_SIMULATOR_TX_J2735_DATA parse_SIMULATOR_TX_payload(uint8_t* data){

            training::St_Payload_SIMULATOR_TX_J2735_DATA payload;
        ...
        static training::St_SIMULATOR_TX_J2735_DATA set_SIMULATOR_TX_DATA(ushort ID, uint8_t* EncodedMsg, int size){

            training::St_SIMULATOR_TX_J2735_DATA sim_tx;

            //sim_tx.payload.UTCTime = set_UTCTime_default();
            sim_tx.payload.UTCTime = set_UTCTime();
            sim_tx.payload.MessageID = ID;
            sim_tx.payload.EncodedMessage = (uint8_t*)malloc(sizeof(uint8_t)*size);
            memcpy(sim_tx.payload.EncodedMessage, EncodedMsg, size);
            sim_tx.payload.EncodedMessageLength = size;
        ...
        static uint8_t* serialize_SERVER_TX_DATA(training::St_SERVER_TX_J2735_DATA data){
        ...
        static void print_SERVER_TX_payload(training::St_Payload_SERVER_TX_J2735_DATA payload){
        ...
        static training::St_Payload_SERVER_TX_J2735_DATA parse_SERVER_TX_payload(uint8_t* data){

            training::St_Payload_SERVER_TX_J2735_DATA payload;
        ...
        static training::St_SERVER_TX_J2735_DATA set_SERVER_TX_BSM(){
            training::St_SERVER_TX_J2735_DATA server_tx;
        ...
        static training::St_SERVER_OR_CLIENT_SIGN set_SIGN(){
            training::St_SERVER_OR_CLIENT_SIGN sign;
        ...
        static training::St_Payload_SIGN parse_SIGN(uint8_t* payload_buf){
            training::St_Payload_SIGN payload_SIGN;
        ...
        static void print_SIGN(training::St_Payload_SIGN payload_sign){
            printf("[SIGN]\n");
        ...
        static uint8_t* serialize_JSON_DATA(training::St_SERVER_OR_CLIENT_JSON_DATA json_data){
        ...
        static training::St_SERVER_OR_CLIENT_JSON_DATA set_JSON_DATA_SPAT(){
            training::St_SERVER_OR_CLIENT_JSON_DATA json_data;
            training::DATA_SPAT spat = set_Json_SPAT();
            const char* json_str_SPAT = get_str_JSON_SPAT(spat);
            training::St_Payload_JSON payload;
        ...
        static training::St_SERVER_OR_CLIENT_JSON_DATA set_JSON_DATA_MAP(){
            training::St_SERVER_OR_CLIENT_JSON_DATA json_data;
            training::DATA_MAP map = set_Json_MAP();
        ...
        static training::St_Header set_header(uint16_t msgType, uint8_t seq, uint16_t payloadLen, uint8_t devType, uint8_t devID[3]){
            training::St_Header header;
        ...
        static training::St_Header parse_header(uint8_t header_buf[9]){
            training::St_Header header;
        ...
        static void print_header(training::St_Header header){
            printf("[Header]\n");
        ...
        static training::St_UTCTime parse_UTCTime(uint8_t* UTCTime_buf){
            training::St_UTCTime UTCTime;
        ...
        static training::St_UTCTime set_UTCTime_default(){
            training::St_UTCTime UTCTime;
            UTCTime.year = 2023;
            UTCTime.month = 1;
            UTCTime.day = 1;
        ...
        static training::St_UTCTime set_UTCTime(){
            training::St_UTCTime UTCTime;
        ...
        static training::DATA_MAP set_Json_MAP(){
            training::DATA_MAP map;
            map.timeStamp = 12;
            map.msgIssueRevision = 12;
            map.layerType = training::LayerType::none;
            map.layerID = 0;

            map.intersections = (training::IntersectionGeometry*)malloc(sizeof(training::IntersectionGeometry)*1);
            ...
            map.intersections[0].speedLimits = (training::RegulatorySpeedLimit*)malloc(sizeof(training::RegulatorySpeedLimit)*1);
            map.intersections[0].speedLimits[0].type = training::SpeedLimitType::vehicleMinSpeed;
            map.intersections[0].speedLimits[0].speed = 0;
            map.intersections[0].laneSet = (training::GenericLane*)malloc(sizeof(training::GenericLane)*1);
            ...
            map.intersections[0].laneSet[0].laneAttributes.laneType.type = training::LaneTypeAttributes_Type::vehicle;
            map.intersections[0].laneSet[0].laneAttributes.laneType.val.vehicle = 128;

            map.intersections[0].laneSet[0].maneuvers = 0b10000000;

            map.intersections[0].laneSet[0].nodeList.type = training::NodeListXY_Type::nodes;
            map.intersections[0].laneSet[0].nodeList.val.nodes = (training::NodeXY*)malloc(sizeof(training::NodeXY)*1);
            map.intersections[0].laneSet[0].nodeList.val.nodes[0].delta.type = training::NodeOffsetPointXY_Type::node_XY1;
            ...
            map.intersections[0].laneSet[0].nodeList.val.nodes[0].attributes.localNode = (training::NodeAttributeXY*)malloc(sizeof(training::NodeAttributeXY)*1);
            map.intersections[0].laneSet[0].nodeList.val.nodes[0].attributes.localNode[0] = training::NodeAttributeXY::stopLine;
            map.intersections[0].laneSet[0].nodeList.val.nodes[0].attributes.disabled = (training::SegmentAttributeXY*)malloc(sizeof(training::SegmentAttributeXY)*1);
            map.intersections[0].laneSet[0].nodeList.val.nodes[0].attributes.disabled[0] = training::SegmentAttributeXY::whiteLine;
            map.intersections[0].laneSet[0].nodeList.val.nodes[0].attributes.enabled = (training::SegmentAttributeXY*)malloc(sizeof(training::SegmentAttributeXY)*1);
            map.intersections[0].laneSet[0].nodeList.val.nodes[0].attributes.enabled[0] = training::SegmentAttributeXY::whiteLine;
            map.intersections[0].laneSet[0].nodeList.val.nodes[0].attributes.data = (training::LaneDataAttribute*)malloc(sizeof(training::LaneDataAttribute)*1);
            map.intersections[0].laneSet[0].nodeList.val.nodes[0].attributes.data[0].type = training::LaneDataAttribute_Type::pathEndPointAngle;
            ...
            map.intersections[0].laneSet[0].connectsTo = (training::Connection*)malloc(sizeof(training::Connection)*1);
            ...
            map.intersections[0].laneSet[0].overlays = (training::LaneID*)malloc(sizeof(training::LaneID)*3);
            map.intersections[0].laneSet[0].overlays[0] = 0;

            map.intersections[0].preemptPriorityData = (training::SignalControlZone*)malloc(sizeof(training::SignalControlZone)*1);
            map.intersections[0].roadAuthorityID.type = training::RoadAuthorityID_Type::fullRdAuthID;
            ...
            map.roadSegments = (training::RoadSegment*)malloc(sizeof(training::RoadSegment)*1);
            ...
            map.roadSegments[0].speedLimits = (training::RegulatorySpeedLimit*)malloc(sizeof(training::RegulatorySpeedLimit)*1);
            map.roadSegments[0].speedLimits[0].type = training::SpeedLimitType::vehicleMinSpeed;
            ...
            map.restrictionList = (training::RestrictionClassAssignment*)malloc(sizeof(training::RestrictionClassAssignment)*1);
            map.restrictionList[0].id = 0;
            map.restrictionList[0].users = (training::RestrictionUserType*)malloc(sizeof(training::RestrictionUserType)*1);
            map.restrictionList[0].users[0].basicType = training::RestrictionAppliesTo::none;

            return map;
        }

        static training::DATA_SPAT set_Json_SPAT(){
            training::DATA_SPAT spat;
            spat.timeStamp = 0;
            strcpy(spat.name, "SPAT");
            spat.intersections = (training::IntersectionState*)malloc(sizeof(training::IntersectionState)*1);
            ...
            spat.intersections[0].enabledLanes = (training::LaneID*)malloc(sizeof(training::LaneID)*3);
            spat.intersections[0].enabledLanes[0] = 0;
            spat.intersections[0].enabledLanes[1] = 0;
            spat.intersections[0].enabledLanes[2] = 0;
            spat.intersections[0].states = (training::MovementState*)malloc(sizeof(training::MovementState)*1);
            strcpy(spat.intersections[0].states[0].movementName, "movement 1");
            spat.intersections[0].states[0].signalGroup = 0;
            spat.intersections[0].states[0].state_time_speed = (training::MovementEvent*)malloc(sizeof(training::MovementEvent)*1);
            spat.intersections[0].states[0].state_time_speed[0].eventState = training::MovementPhaseState::unavailable;
            ...
            spat.intersections[0].states[0].state_time_speed[0].speeds = (training::AdvisorySpeed*)malloc(sizeof(training::AdvisorySpeed)*1);
            spat.intersections[0].states[0].state_time_speed[0].speeds[0].type = training::AdvisorySpeedType::none;
            spat.intersections[0].states[0].state_time_speed[0].speeds[0].speed = 0;
            spat.intersections[0].states[0].state_time_speed[0].speeds[0].confidence = training::SpeedConfidence::unavailable;
            ...
            spat.intersections[0].states[0].maneuverAssistList = (training::ConnectionManeuverAssist*)malloc(sizeof(training::ConnectionManeuverAssist)*1);
            ...
            spat.intersections[0].maneuverAssistList = (training::ConnectionManeuverAssist*)malloc(sizeof(training::ConnectionManeuverAssist)*1);
            ...
            spat.intersections[0].roadAuthorityID.type = training::RoadAuthorityID_Type::fullRdAuthID;
        ...
        static void print_Json_SPAT(training::DATA_SPAT spat){
        ...
        static const char* get_str_JSON_SPAT(training::DATA_SPAT spat){
            ...
            int siz = malloc_usable_size(spat.intersections[0].enabledLanes)/sizeof(training::LaneID*);
            for(int i = 0; i<siz; i++){
        ...
        static const char* get_str_JSON_MAP(training::DATA_MAP map){
            json_object* root = json_object_new_object();
            ...
            int siz = malloc_usable_size(map.intersections[0].laneSet[0].overlays)/sizeof(training::LaneID*);
            for(int i = 0; i<siz; i++){
        ...
        static void print_Json_MAP(training::DATA_MAP map){
        ...
        static training::St_Payload_JSON parse_json_payload(uint8_t* payload){
            training::St_Payload_JSON payload_json;
        ...
        static void print_UTCTime(training::St_UTCTime UTCTime){
            printf("year: %d, month: %d, day: %d\n", UTCTime.year, UTCTime.month, UTCTime.day);
            printf("hour: %d, min: %d, msec: %05d\n", UTCTime.hour, UTCTime.min, UTCTime.msec);
        }

        static void print_json_payload(training::St_Payload_JSON payload_json){
            printf("[Json Payload]\n");
        ...
        static void print_VehicleClassification(training::VehicleClassification obj){
            printf("keyType: %d\n", (int)obj.keyType);
            printf("role: %d\n", (int)obj.role);
            printf("iso3883: %d\n", (int)obj.iso3883);
            printf("hpmsType: %d\n", (int)obj.hpmsType);
        ...
        static void print_FullPositionVector(training::FullPositionVector fullPositionVector){
        ...
        static void print_Snapshots(training::SnapshotList snapshots){
            for(int i = 0; i<snapshots.size; i++){
                print_FullPositionVector(snapshots.val[i].thePosition);
            }
        }

        static void print_Json_PVD(training::DATA_PVD pvd){
            ...
            if(pvd.probeID.id.type == training::VehicleID_type::entityID){
        ...
        static training::DATA_PVD parse_json_pvd(uint8_t* JsonMessage){
            training::DATA_PVD Json_pvd;           

            json_object* msg = json_tokener_parse((const char*)JsonMessage);
            json_object* null = json_object_new_null();

            json_object* obj = json_object_object_get(msg, "value");
            ...
            if (json_object_equal(probeID_id, null) != 1){
                ...
                Json_pvd.probeID.id.type = (training::VehicleID_type)json_object_get_int(id_type);
            ...
            if (json_object_equal(probeID_vehicleType, null) != 1){
                Json_pvd.probeID.vehicleType = (training::VehicleType)json_object_get_int(probeID_vehicleType);
            }
            ...
            if (json_object_equal(probeID_vehicleClass, null) != 1){
                ...
                Json_pvd.probeID.vehicleClass.type = (training::vehicleClass_type)json_object_get_int(JO_vehicleClass_type);

                json_object* JO_vehicleClass_val = json_object_object_get(probeID_vehicleClass, "val");
                if (Json_pvd.probeID.vehicleClass.type == (training::vehicleClass_type::vGroup)){
                    json_object* vehicleClass_val_vGroup = json_object_object_get(JO_vehicleClass_val, "vGroup");
                    Json_pvd.probeID.vehicleClass.val.vGroup = (training::VehicleGroupAffected)json_object_get_int(vehicleClass_val_vGroup);
                }
                else if (Json_pvd.probeID.vehicleClass.type == (training::vehicleClass_type::rGroup)){
                    json_object* vehicleClass_val_rGroup = json_object_object_get(JO_vehicleClass_val, "rGroup");
                    Json_pvd.probeID.vehicleClass.val.rGroup = (training::ResponderGroupAffected)json_object_get_int(vehicleClass_val_rGroup);
                }
                else if (Json_pvd.probeID.vehicleClass.type == (training::vehicleClass_type::rEquip)){
                    json_object* vehicleClass_val_rEquip = json_object_object_get(JO_vehicleClass_val, "rEquip");
                    Json_pvd.probeID.vehicleClass.val.rEquip = (training::IncidentResponseEquipment)json_object_get_int(vehicleClass_val_rEquip);
                }
            }
            else{
                Json_pvd.probeID.vehicleClass.type = (training::vehicleClass_type)-1;
            }
            ...
            if (json_object_equal(JO_vehicleType, null) != 1){
                Json_pvd.vehicleType.keyType = (training::BasicVehicleClass)json_object_get_int(json_object_object_get(JO_vehicleType, "keyType"));
                Json_pvd.vehicleType.role = (training::BasicVehicleRole)json_object_get_int(json_object_object_get(JO_vehicleType, "role"));
                Json_pvd.vehicleType.iso3883 = (training::Iso3833VehicleType)json_object_get_int(json_object_object_get(JO_vehicleType, "iso3883"));
                Json_pvd.vehicleType.hpmsType = (training::VehicleType)json_object_get_int(json_object_object_get(JO_vehicleType, "hpmsType"));
                Json_pvd.vehicleType.vehicleType = (training::VehicleGroupAffected)json_object_get_int(json_object_object_get(JO_vehicleType, "vehicleType"));
                Json_pvd.vehicleType.responseEquip = (training::IncidentResponseEquipment)json_object_get_int(json_object_object_get(JO_vehicleType, "responseEquip"));
                Json_pvd.vehicleType.responderType = (training::ResponderGroupAffected)json_object_get_int(json_object_object_get(JO_vehicleType, "responderType"));
                Json_pvd.vehicleType.fuelType = (training::FuelType)json_object_get_int(json_object_object_get(JO_vehicleType, "fuelType"));
            }
            ...
            if (json_object_equal(JO_snapshots, null) != 1){
                ...
                Json_pvd.snapshots.val = (training::Snapshot*)malloc(sizeof(training::Snapshot)*Json_pvd.snapshots.size);
                        ...
                        if (json_object_equal(JO_snapshot_safetyExt, null) != 1){
                            ...
                            if (json_object_equal(JO_safetyExt_pathHistory, null) != 1){
                                ...
                                if (json_object_equal(JO_pathHistory_crumbData, null) != 1){
                                    ...
                                    Json_pvd.snapshots.val[i].safetyExt.pathHistory.crumbData = (training::PathHistoryPoint*)malloc(sizeof(training::PathHistoryPoint)*crumbData_len);
                                    for(int j = 0; j < crumbData_len; j++){
                                        ...
                                        if (json_object_equal(JO_crumbData, null) != 1){
                                            ...
                                            Json_pvd.snapshots.val[i].safetyExt.pathHistory.crumbData[j].latOffset = (training::OffsetLL_B18)json_object_get_int(JO_crumbData_latOffset);

                                            json_object* JO_crumbData_lonOffset = json_object_object_get(JO_crumbData, "lonOffset");
                                            Json_pvd.snapshots.val[i].safetyExt.pathHistory.crumbData[j].lonOffset = (training::OffsetLL_B18)json_object_get_int(JO_crumbData_lonOffset);

                                            json_object* JO_crumbData_elevationOffset = json_object_object_get(JO_crumbData, "elevationOffset");
                                            Json_pvd.snapshots.val[i].safetyExt.pathHistory.crumbData[j].elevationOffset = (training::VertOffset_B12)json_object_get_int(JO_crumbData_elevationOffset);

                                            json_object* JO_crumbData_timeOffset = json_object_object_get(JO_crumbData, "timeOffset");
                                            Json_pvd.snapshots.val[i].safetyExt.pathHistory.crumbData[j].timeOffset = (training::TimeOffset)json_object_get_int(JO_crumbData_timeOffset);

                                            json_object* JO_crumbData_speed = json_object_object_get(JO_crumbData, "speed");
                                            Json_pvd.snapshots.val[i].safetyExt.pathHistory.crumbData[j].speed = (training::Speed)json_object_get_int(JO_crumbData_speed);
                                            ...
                                            Json_pvd.snapshots.val[i].safetyExt.pathHistory.crumbData[j].heading = (training::CoarseHeading)json_object_get_int(JO_crumbData_heading);
                                        }
                                    }
                                }
                            ...
                            if (json_object_equal(JO_safetyExt_pathPrediction, null) != 1){
                                Json_pvd.snapshots.val[i].safetyExt.pathPrediction.radiusOfCurve = (training::RadiusOfCurvature)json_object_get_int(json_object_object_get(JO_safetyExt_pathPrediction, "radiusOfCurve"));
                                Json_pvd.snapshots.val[i].safetyExt.pathPrediction.confidence = (training::Confidence)json_object_get_int(json_object_object_get(JO_safetyExt_pathPrediction, "confidence"));
                            ...
                            if (json_object_equal(JO_safetyExt_lights, null) != 1){
                                Json_pvd.snapshots.val[i].safetyExt.lights = (training::ExteriorLights)json_object_get_int(JO_safetyExt_lights);
                            ...
                            if (json_object_equal(JO_safetyExt_height, null) != 1){
                                Json_pvd.snapshots.val[i].safetyExt.height = (training::VehicleHeight)json_object_get_int(JO_safetyExt_height);
                        ...
                        if (json_object_equal(JO_snapshot_dataSet, null) != 1){
                            ...
                            if(0){
                                ...
                                if (json_object_equal(dataSet_lights, null) != 1){
                                    Json_pvd.snapshots.val[i].dataSet.lights = (training::ExteriorLights)json_object_get_int(dataSet_lights);
                                }
                                ...
                                if (json_object_equal(dataSet_lightBar, null) != 1){
                                    Json_pvd.snapshots.val[i].dataSet.lightBar = (training::LightbarInUse)json_object_get_int(dataSet_lightBar);
                                }
                                ...
                                if (json_object_equal(dataSet_brakePressure, null) != 1){
                                    //Json_pvd.snapshots.val[i].dataSet.brakePressure = (training::LightbarInUse)json_object_get_int(dataSet_lightBar);
                                }
                                ...
                                if (json_object_equal(dataSet_roadFriction, null) != 1){
                                    //Json_pvd.snapshots.val[i].dataSet.roadFriction = (training::LightbarInUse)json_object_get_int(dataSet_lightBar);
                                }
                                ...
                                if (json_object_equal(dataSet_sunData, null) != 1){
                                    //Json_pvd.snapshots.val[i].dataSet.sunData = (training::LightbarInUse)json_object_get_int(dataSet_lightBar);
                                }
                                ...
                                if (json_object_equal(dataSet_rainData, null) != 1){
                                    //Json_pvd.snapshots.val[i].dataSet.rainData = (training::LightbarInUse)json_object_get_int(dataSet_lightBar);
                                }
                                ...
                                if (json_object_equal(dataSet_airTemp, null) != 1){
                                    //Json_pvd.snapshots.val[i].dataSet.airTemp = (training::LightbarInUse)json_object_get_int(dataSet_lightBar);
                                }
                                ...
                                if (json_object_equal(dataSet_airPres, null) != 1){
                                    //Json_pvd.snapshots.val[i].dataSet.airPres = (training::LightbarInUse)json_object_get_int(dataSet_lightBar);
                                }
                                ...
                                if (json_object_equal(dataSet_steering, null) != 1){
                                    //Json_pvd.snapshots.val[i].dataSet.steering = (training::LightbarInUse)json_object_get_int(dataSet_lightBar);
                                }
                                ...
                                if (json_object_equal(dataSet_accelSets, null) != 1){
                                    //Json_pvd.snapshots.val[i].dataSet.accelSets = (training::LightbarInUse)json_object_get_int(dataSet_lightBar);
                                }
                                json_object* dataSet_Object = json_object_object_get(JO_snapshot_dataSet, "Object");
                                if (json_object_equal(dataSet_Object, null) != 1){
                                    //Json_pvd.snapshots.val[i].dataSet.Object = (training::LightbarInUse)json_object_get_int(dataSet_lightBar);
                                }
                                ...
                                if (json_object_equal(dataSet_fullPos, null) != 1){
                                    //Json_pvd.snapshots.val[i].dataSet.fullPos = (training::LightbarInUse)json_object_get_int(dataSet_lightBar);
                                }
                                ...
                                if (json_object_equal(dataSet_throttlePos, null) != 1){
                                    //Json_pvd.snapshots.val[i].dataSet.throttlePos = (training::LightbarInUse)json_object_get_int(dataSet_lightBar);
                                }
                                ...
                                if (json_object_equal(dataSet_speedHeadC, null) != 1){
                                    //Json_pvd.snapshots.val[i].dataSet.speedHeadC = (training::LightbarInUse)json_object_get_int(dataSet_lightBar);
                                }
                                json_object* dataSet_speedC = json_object_object_get(JO_snapshot_dataSet, "speedC");
                                if (json_object_equal(dataSet_speedC, null) != 1){
                                    //Json_pvd.snapshots.val[i].dataSet.speedC = (training::LightbarInUse)json_object_get_int(dataSet_lightBar);
                                }
                                ...
                                if (json_object_equal(dataSet_vehicleData, null) != 1){
                                    //Json_pvd.snapshots.val[i].dataSet.vehicleData = (training::LightbarInUse)json_object_get_int(dataSet_lightBar);
                                }
                                ...
                                if (json_object_equal(dataSet_vehicleIdent, null) != 1){
                                    //Json_pvd.snapshots.val[i].dataSet.vehicleIdent = (training::LightbarInUse)json_object_get_int(dataSet_lightBar);
                                }
                                ...
                                if (json_object_equal(dataSet_j1939data, null) != 1){
                                    //Json_pvd.snapshots.val[i].dataSet.j1939data = (training::LightbarInUse)json_object_get_int(dataSet_lightBar);
                                }
                                ...
                                if (json_object_equal(dataSet_weatherReport, null) != 1){
                                    //Json_pvd.snapshots.val[i].dataSet.weatherReport = (training::LightbarInUse)json_object_get_int(dataSet_lightBar);
                                }
                                ...
                                if (json_object_equal(dataSet_gnssStatus, null) != 1){
                                    //Json_pvd.snapshots.val[i].dataSet.gnssStatus = (training::LightbarInUse)json_object_get_int(dataSet_lightBar);
                                }
                            }
                        }
                    }
        ...
        static training::DDateTime parse_Json_DDateTime(json_object* JO_DDateTime){
            training::DDateTime dDateTime;
            json_object* null = json_object_new_null();
        ...
        static training::FullPositionVector parse_Json_FullPositionVector(json_object* JO_fullPositionVector){

            json_object* null = json_object_new_null();
            training::FullPositionVector fullPositionVector;
            ...
            if (json_object_equal(JO_fullPositionVector, null) != 1){
                ...
                if (json_object_equal(JO_fullPositionVector_speed, null) != 1){
                    fullPositionVector.speed.transmission = (training::TransmissionState)json_object_get_int(json_object_object_get(JO_fullPositionVector_speed, "transmission"));
                ...
                if (json_object_equal(JO_fullPositionVector_posAccuracy, null) != 1){
                    fullPositionVector.posAccuracy.semiMajor = (training::SemiMajorAxisAccuracy)json_object_get_int(json_object_object_get(JO_fullPositionVector_posAccuracy, "semiMajor"));
                    fullPositionVector.posAccuracy.semiMinor = (training::SemiMinorAxisAccuracy)json_object_get_int(json_object_object_get(JO_fullPositionVector_posAccuracy, "semiMinor"));
                    fullPositionVector.posAccuracy.orientation = (training::SemiMajorAxisOrientation)json_object_get_int(json_object_object_get(JO_fullPositionVector_posAccuracy, "orientation"));
                }
                else{
                ...
                if (json_object_equal(JO_fullPositionVector_timeConfidence, null) != 1){
                    fullPositionVector.timeConfidence = (training::TimeConfidence)json_object_get_int(JO_fullPositionVector_timeConfidence);
                }
                else{
                    fullPositionVector.timeConfidence = (training::TimeConfidence)0;
                }
                ...
                if (json_object_equal(JO_fullPositionVector_posConfidence, null) != 1){
                    fullPositionVector.posConfidence.pos = (training::PositionConfidence)json_object_get_int(json_object_object_get(JO_fullPositionVector_posConfidence, "pos"));
                    fullPositionVector.posConfidence.elevation = (training::ElevationConfidence)json_object_get_int(json_object_object_get(JO_fullPositionVector_posConfidence, "elevation"));
                }
                else{
                    fullPositionVector.posConfidence.pos = (training::PositionConfidence)0;
                    fullPositionVector.posConfidence.elevation = (training::ElevationConfidence)0;
                }
                ...
                if (json_object_equal(JO_fullPositionVector_speedConfidence, null) != 1){
                    fullPositionVector.speedConfidence.heading = (training::HeadingConfidence)json_object_get_int(json_object_object_get(JO_fullPositionVector_speedConfidence, "heading"));
                    fullPositionVector.speedConfidence.speed = (training::SpeedConfidence)json_object_get_int(json_object_object_get(JO_fullPositionVector_speedConfidence, "speed"));
                    fullPositionVector.speedConfidence.throttle = (training::ThrottleConfidence)json_object_get_int(json_object_object_get(JO_fullPositionVector_speedConfidence, "throttle"));
                }
                else{
                    fullPositionVector.speedConfidence.heading = (training::HeadingConfidence)0;
                    fullPositionVector.speedConfidence.speed = (training::SpeedConfidence)0;
                    fullPositionVector.speedConfidence.throttle = (training::ThrottleConfidence)0;
                }
            }
        ...
        static training::DATA_BSM parse_json_bsm(uint8_t* JsonMessage){
            training::DATA_BSM Json_bsm;           
            ...
            Json_bsm.coreData.transmission = (training::TransmissionState)json_object_get_int(transmission);
            ...
            Json_bsm.coreData.brakes.traction = (training::TractionControlStatus)json_object_get_int(brakes_traction);
            Json_bsm.coreData.brakes.abs = (training::AntiLockBrakeStatus)json_object_get_int(brakes_abs);
            Json_bsm.coreData.brakes.scs = (training::StabilityControlStatus)json_object_get_int(brakes_scs);
            Json_bsm.coreData.brakes.brakeBoost = (training::BrakeBoostApplied)json_object_get_int(brakes_brakeBoost);
            Json_bsm.coreData.brakes.auxBrakes = (training::AuxiliaryBrakeStatus)json_object_get_int(brakes_auxBrakes);                
        ...
        static void print_json_bsm(training::DATA_BSM Json_bsm){
            printf("[Json BSM]\n");
